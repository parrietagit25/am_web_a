...
<head>
    ...
    <style>
        ...
        #ptz-frame { width: 100%; min-height: 520px; border: 1px solid var(--border-color); border-radius: 12px; background: #fff; }
...
<div class="col-lg-9 col-md-8 p-0">
    ...
    <div class="p-4">
        ...
        <?php if ($lastPayment !== null): ?>
        <div class="admin-card">
            <h2 class="h5 fw-bold mb-3">Último pago de prueba</h2>
            <div class="row g-2 small">
                <div class="col-md-3"><strong>ID:</strong> <?php echo esc((string) ($lastPayment['payment_id'] ?? '')); ?></div>
                <div class="col-md-3"><strong>Referencia:</strong> <?php echo esc((string) ($lastPayment['test_reference'] ?? '')); ?></div>
                <div class="col-md-3"><strong>Estado:</strong> <?php echo esc((string) ($lastPayment['status'] ?? '')); ?></div>
                <div class="col-md-3"><strong>ISO:</strong> <?php echo esc((string) ($lastPayment['iso_response_code'] ?? '')); ?></div>
                <div class="col-md-12"><strong>Mensaje:</strong> <?php echo esc((string) ($lastPayment['response_message'] ?? '')); ?></div>
            </div>
        </div>
        <?php endif; ?>
        ...
        <div class="admin-card">
            ...
            <div class="row g-2 small mb-3">
                <div class="col-md-3"><strong>payment_id:</strong> <span id="ptz-payment-id">—</span></div>
                <div class="col-md-3"><strong>test_reference:</strong> <span id="ptz-reference">—</span></div>
                <div class="col-md-3"><strong>status:</strong> <span id="ptz-status" class="badge bg-secondary status-pill">—</span></div>
                <div class="col-md-3"><strong>ISO:</strong> <span id="ptz-iso">—</span></div>
                <div class="col-md-3"><strong>Auth code:</strong> <span id="ptz-auth">—</span></div>
                <div class="col-md-12"><strong>Mensaje:</strong> <span id="ptz-message">—</span></div>
                <div class="col-md-6"><strong>order_identifier:</strong> <span id="ptz-order">—</span></div>
                <div class="col-md-6"><strong>transaction_identifier:</strong> <span id="ptz-txn">—</span></div>
            </div>
            <iframe id="ptz-frame" title="Powertranz HPP" sandbox="allow-forms allow-scripts allow-same-origin allow-popups allow-top-navigation"></iframe>
        </div>
    </div>
</div>
</div></div>
...
(function () {
    ...
    function applyPayment(data) {
        ...
        setField('ptz-status', data.status || '—');
        ...
        if (badge) {
            badge.className = 'badge status-pill bg-' + (data.approved ? 'success' : (data.status === 'declined' ? 'danger' : 'secondary'));
        }
    }
    ...
    async function fetchStatus() {
        ...
        applyPayment(data);
        showAlert(data.ok ? 'info' : 'warning', 'Estado actualizado: ' + (data.status || '—'));
    }
    ...
    form.addEventListener('submit', async function (e) {
        ...
        try {
            ...
            applyPayment(data);
            if (data.hpp_warning) {
                showAlert('warning', data.hpp_warning);
            }
            if (data.ok && data.frame_url) {
                frame.src = data.frame_url;
                showAlert('success', 'Pago iniciado. Complete el flujo HPP/3DS en el iframe.');
            } else {
                showAlert('danger', data.message || data.error || 'No se pudo iniciar el pago.');
